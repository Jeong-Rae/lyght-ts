...
import { extractDateFromFilename, isOlderThan, toDateString } from '../utils/datetime';
import {
  compressFile,
  createWriteStream,
  deleteFile,
...
export class FileTransport implements Transport {
  ...
  private handleSizeBasedLogging(formattedEntry: string): void {
    ...
    // 파일 크기 체크 및 회전
    if (this.currentFileSize + Buffer.byteLength(formattedEntry) > this.maxFileSize) {
  ...
  private handleDateBasedLogging(formattedEntry: string, timestamp: Date): void {
  ...
  private handleCombinedLogging(formattedEntry: string, timestamp: Date): void {
    ...
    if (this.currentStream) {
      ...
      if (currentSize + Buffer.byteLength(formattedEntry) > this.maxFileSize) {
        ...
        globalBackgroundQueue.enqueue(() => this.rotateDateBasedFile(currentFilePath, timestamp));
      }
  ...
  private getCurrentFilePath(date: Date): string {
    const formattedDate = toDateString(date);
    return path.join(this.filePath, `${this.fileNamePattern}-${formattedDate}${LOG_FILE_EXTENSION}`);
  }
  ...
  private async rotateDateBasedFile(filePath: string, timestamp: Date): Promise<void> {
    try {
      // 현재 스트림이 해당 파일을 사용 중이면 종료
      if (this.currentStream && this.getCurrentFilePath(timestamp) === filePath) {
        this.currentStream.end();
        this.currentStream = undefined;
        this.currentDate = '';
      }
      ...
      let rotatedPath = path.join(dir, `${basename}.${rotationNumber}${extension}`);

      // 사용 가능한 번호 찾기
      while (exists(rotatedPath)) {
        rotationNumber++;
        rotatedPath = path.join(dir, `${basename}.${rotationNumber}${extension}`);
      }
  ...
  private cleanupOldFiles(): void {
    try {
      if (this.maxDays) {
        ...
        for (const file of files) {
          ...
          if (match) {
            ...
            if (fileDate && isOlderThan(fileDate, this.maxDays as number, currentDate)) {
      ...
      if (this.maxFiles && (this.rotation === 'size' || this.rotation === 'hybrid')) {
        // 파일 개수 기반 정리 (크기 기반 롤링 파일들)
        const dir = this.rotation === 'size' ? getDirectory(this.filePath) : this.filePath;
        const files = listFiles(dir);
        ...
        // 최대 개수 초과 파일 삭제
        for (const { file } of numberedFiles.slice(this.maxFiles)) {
