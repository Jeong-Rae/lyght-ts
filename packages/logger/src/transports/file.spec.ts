...
import { FileTransport, FileTransportOptions, RotationTrigger } from "./file";
...
describe("FileTransport", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(fsUtils.getDirectory).mockImplementation((p) =>
			p.split("/").slice(0, -1).join("/"),
		);
	...
	describe("다양한 설정 조합", () => {
		it("모든 옵션을 포함한 하이브리드 설정", () => {
			...
			expect(fsUtils.createWriteStream).toHaveBeenCalledWith(
				"./logs/complex-app-2024-01-15.log",
				{ flags: "a" },
			);
		});
	});
