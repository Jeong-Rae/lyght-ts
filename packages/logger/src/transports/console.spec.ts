...
describe('ConsoleTransport', () => {
  ...
  describe('log', () => {
    it('debug 레벨 로그를 console.debug로 출력합니다', () => {
      ...
      expect(console.debug).toHaveBeenCalledWith('2024-01-01T00:00:00.000Z [DEBUG] debug message {"userId":"123"}');
    });
    ...
    it('info 레벨 로그를 console.info로 출력합니다', () => {
      ...
      expect(console.info).toHaveBeenCalledWith('2024-01-01T12:30:45.123Z [INFO] info message {"requestId":"abc123"}');
    });
    ...
    it('warn 레벨 로그를 console.warn으로 출력합니다', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('2024-01-01T09:15:30.456Z [WARN] warning message');
    });
    ...
    it('meta가 없을 때 빈 객체로 처리합니다', () => {
      ...
      expect(console.info).toHaveBeenCalledWith('2024-01-01T00:00:00.000Z [INFO] simple message');
    });
    ...
    it('각 로그 레벨에 대해 올바른 console 메서드를 호출합니다', () => {
      ...
      const testCases: [LogLevel, string][] = [
        ['debug', 'debug test'],
        ['info', 'info test'],
        ['warn', 'warn test'],
        ['error', 'error test'],
      ...
      for (const [level, message] of testCases) {
        consoleTransport.log(level, message, {}, mockDate);
      }

      expect(console.debug).toHaveBeenCalledWith('2024-01-01T00:00:00.000Z [DEBUG] debug test');
      expect(console.info).toHaveBeenCalledWith('2024-01-01T00:00:00.000Z [INFO] info test');
      expect(console.warn).toHaveBeenCalledWith('2024-01-01T00:00:00.000Z [WARN] warn test');
      expect(console.error).toHaveBeenCalledWith('2024-01-01T00:00:00.000Z [ERROR] error test');
    });
