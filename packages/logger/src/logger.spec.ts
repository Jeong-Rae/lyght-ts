...
describe('Logger', () => {
  ...
  describe('getLevel', () => {
    ...
    it('환경변수에서 로그 레벨을 가져옵니다', () => {
      ...
      expect(mockTransport.log).toHaveBeenCalledWith('warn', 'test message', {}, expect.any(Date));
    });
    ...
    it('잘못된 로그 레벨일 때 기본값(info)을 사용합니다', () => {
      ...
      Logger.info('test message');
      expect(mockTransport.log).toHaveBeenCalledWith('info', 'test message', {}, expect.any(Date));
    });
  });
  ...
  describe('useTransports', () => {
    it('여러 개의 transport를 설정할 수 있습니다', () => {
      ...
      expect(transport1.log).toHaveBeenCalledWith('info', 'test message', {}, expect.any(Date));
      expect(transport2.log).toHaveBeenCalledWith('info', 'test message', {}, expect.any(Date));
    });
  });
  ...
  describe('transport error handling', () => {
    it('Transport에서 에러가 발생해도 다른 Transport는 계속 동작합니다', () => {
      ...
      expect(workingTransport.log).toHaveBeenCalledWith('info', 'test message', {}, expect.any(Date));
    });
  });
  ...
  describe('debug', () => {
    it('debug 레벨 로그를 출력합니다', () => {
      ...
      expect(mockTransport.log).toHaveBeenCalledWith('debug', 'debug message', meta, expect.any(Date));
    });
    ...
    it('meta 없이 debug 로그를 출력합니다', () => {
      ...
      expect(mockTransport.log).toHaveBeenCalledWith('debug', 'debug message', {}, expect.any(Date));
    });
  });
  ...
  describe('info', () => {
    it('info 레벨 로그를 출력합니다', () => {
      ...
      expect(mockTransport.log).toHaveBeenCalledWith('info', 'info message', meta, expect.any(Date));
    });
  });
  ...
  describe('warn', () => {
    it('warn 레벨 로그를 출력합니다', () => {
      ...
      expect(mockTransport.log).toHaveBeenCalledWith('warn', 'warn message', meta, expect.any(Date));
    });
  });
  ...
  describe('error', () => {
    it('문자열 에러 메시지로 error 로그를 출력합니다', () => {
      ...
      expect(mockTransport.log).toHaveBeenCalledWith('error', 'error message', meta, expect.any(Date));
    });
